...
| 成员 | 分工模块 | 职责 |
|---|---|---|
| 组长：桂佳锋 | 需求分析 / 系统设计 / 整体架构 | WBS 工作分解、Use-Case 用例图、UML 类图、整体架构设计、进度统筹与验收 |
| 组员：高彦霄 | 角色 / 敌人模块 | `creature.h/.cpp`、属性与成长、敌人 AI、职业差异化实现 |
| 组员：高树森 | 物品 / 装备模块 | `item.h/.cpp`、掉落与工厂、背包管理、装备强化属性 |
| 组员：贾庭昊 | 地图 / 房间 / 存档模块 | `room.h/.cpp`、`world.h/.cpp`、关卡配置、存档读档 |
| 组员：李王鸿戈 | 战斗 / 命令 / 测试模块 | `game.h/.cpp`、回合制战斗、命令解析、自动化测试与测试报告 |
